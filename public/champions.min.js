...
function isCharacterActive(characterId){return db.characters.get(characterId).then(character=>character.active);return db.characters.where('id').equals(character).first().then(character=>character.active)}
...
function setCurrentCampaign(campaign){return db.parameters.get('currentCampaign').then(currentCampaign=>{return db.parameters.put(currentCampaign===campaign?0:campaign,'currentCampaign')})}
...
function sortByDex(a,b){if(a.dex===b.dex){if(a.speed===b.speed){return sortByName(a,b)}
